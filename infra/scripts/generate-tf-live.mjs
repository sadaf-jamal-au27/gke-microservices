#!/usr/bin/env node
/**
 * Generates pure Terraform live stacks (no Terragrunt):
 * infra/terraform/live/<env>/<stack>/{backend,versions,provider,variables,main,outputs}.tf + <stack>.tfvars
 */
...
const liveRoot = path.join(infraRoot, "terraform", "live");
const scaffoldDir = path.join(infraRoot, "terraform", "modules", "_scaffold");
const tgEnvRoot = path.join(infraRoot, "terragrunt", "environments");
...
function outputsTf(stack) {
  const name = stack === "project_services" ? "project_services" : stack;
  const map = {
...
for (const env of ENVIRONMENTS) {
  const envDir = path.join(liveRoot, env.name);
  fs.mkdirSync(envDir, { recursive: true });

  fs.writeFileSync(
    path.join(envDir, "env.tfvars"),
...
`
  ...
  for (const stack of STACKS) {
    const stackDir = path.join(envDir, stack);
    ...
    const legacyTfvars = path.join(tgEnvRoot, env.name, stack, `${stack}.tfvars`);
    ...
    if (fs.existsSync(legacyTfvars)) {
      ...
      fs.writeFileSync(targetTfvars, `# Module-specific overrides for ${stack}\n`);
    }
    if (stack === "github_wif") {
      fs.appendFileSync(
        targetTfvars,
...
console.log("Generated infra/terraform/live/{dev,qa,test,prod}/<stack>/ (pure Terraform)");
