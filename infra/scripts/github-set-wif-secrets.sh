...
if [[ -z "${WIF_PROVIDER}" || -z "${CI_SA}" ]]; then
  echo "Run from gke-retail-infra (or monorepo infra/) with github_wif applied."
  echo "  ./scripts/tf.sh ${ENV} github_wif apply"
  exit 1
fi
...
set_secret() {
  ...
  if [[ -n "${TF_VAR_DATABASE_PASSWORD:-}" ]]; then
    ...
    echo "  (skip TF_VAR_DATABASE_PASSWORD — export TF_VAR_DATABASE_PASSWORD to set)"
  fi
}
...
echo "Done. Optional repo variables (non-secret):"
echo "  gh variable set GCP_PROJECT_ID --repo ${ORG}/gke-retail-infra --body ${PROJECT_ID}"
