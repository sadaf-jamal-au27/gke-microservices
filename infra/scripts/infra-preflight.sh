...
if gcloud storage buckets describe "gs://${STATE_BUCKET}" >/dev/null 2>&1; then
  ...
  echo "MISSING: state bucket — run ./infra/scripts/gcp-bootstrap.sh ${ENV}"
fi

GITHUB_ORG="$(grep '^github_org' "${ENV_TFVARS}" | head -1 | cut -d'"' -f2 || true)"
if [[ "${GITHUB_ORG}" == REPLACE_* || -z "${GITHUB_ORG}" ]]; then
  echo "WARN: github_org not set in env.tfvars (needed before github_wif apply)"
else
  echo "OK: github_org=${GITHUB_ORG}"
fi

if [[ -z "${TF_VAR_database_password:-}" ]]; then
  echo "WARN: TF_VAR_database_password not set (required for cloudsql apply)"
else
  echo "OK: TF_VAR_database_password is set"
fi

echo ""
echo "Next: ./infra/scripts/tf-apply-all.sh ${ENV} plan"
